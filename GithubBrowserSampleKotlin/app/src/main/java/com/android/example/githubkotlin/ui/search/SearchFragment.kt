...
package com.android.example.githubkotlin.ui.search
...
import com.android.example.githubkotlin.binding.FragmentDataBindingComponent
import com.android.example.githubkotlin.databinding.SearchFragmentBinding
import com.android.example.githubkotlin.di.Injectable
import com.android.example.githubkotlin.ui.common.NavigationController
import com.android.example.githubkotlin.ui.common.RepoListAdapter
import com.android.example.githubkotlin.ui.common.RetryCallback
import com.android.example.githubkotlin.util.AutoClearedValue
import com.android.example.githubkotlin.vo.Repo
import com.android.example.githubkotlin.vo.Resource
    ...
    private fun initRecyclerView() {
        ...
        searchViewModel.results.observe(
                ...
                Observer { result: Resource<List<Repo>>? ->
                    binding.get()!!.run {
                        searchResource = result
                        resultCount = when {
                            result?.data == null -> 0
                            else -> result.data.size
                        }
                        adapter.get()!!.setList(result?.data)
                        executePendingBindings()
                    }
                })

        searchViewModel.loadMoreStatus.observe(this, Observer { more ->
            binding.get()!!.run {
                if (more == null) {
                    loadingMore = false
                } else {
                    loadingMore = more.isRunning
